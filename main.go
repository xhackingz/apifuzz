...
func ParseFlags(opts *ffuf.ConfigOptions) *ffuf.ConfigOptions {
	...
	flag.StringVar(&opts.Output.OutputFile, "o", opts.Output.OutputFile, "Write output to file, or terminal mode: normal, live, silent")
	flag.StringVar(&opts.Output.OutputFormat, "of", opts.Output.OutputFormat, "Output file format: json, csv, md (default: json)")
	flag.StringVar(&opts.Output.OutputMode, "om", opts.Output.OutputMode, "Terminal output mode: normal, live, silent")
	flag.StringVar(&opts.Output.OutputMode, "output-mode", opts.Output.OutputMode, "Terminal output mode: normal, live, silent")
	...
	if isOutputMode(opts.Output.OutputFile) {
		opts.Output.OutputMode = strings.ToLower(strings.TrimSpace(opts.Output.OutputFile))
...
func main() {
	...
	if !conf.Quiet && conf.OutputMode == "normal" {
		out.Banner()
	}
