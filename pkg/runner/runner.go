...
func NewSimpleRunner(conf *ffuf.Config) *SimpleRunner {
	...
	return &SimpleRunner{
		...
		sizeCounters: make(map[string]int64),
	}
}
...
func (r *SimpleRunner) Run(out ffuf.OutputProvider) error {
	...
	for _, wl := range wordlistSources {
		...
		if !r.conf.Quiet && r.conf.OutputMode == "normal" {
	...
	if r.conf.AutoCalibration {
		if len(targets) == 1 {
			...
			if err := r.autoCalibrate(out); err != nil && !r.conf.Quiet && r.conf.OutputMode == "normal" {
	...
	if !r.conf.Quiet && !r.conf.Json && r.conf.OutputMode == "normal" {
			...
			case <-ticker.C:
				if !r.conf.Quiet && !r.conf.Json && r.conf.OutputMode != "silent" {
					...
					if r.conf.OutputMode == "live" {
						out.Progress(cur, int64(total), found, errs, rps)
					} else {
		...
		go func() {
			...
			for {
				...
				case job, ok := <-jobs:
					...
					if passed, reason := filter.PassesFilters(r.conf, &res); !passed {
						if r.conf.Verbose {
					...
					if b, ok := perDomainBaselines[job.URLTemplate]; ok {
						if matchesBaseline(&res, b) {
							seen := r.recordSizeSeen(res)
							out.CatchAll(res, seen)
							if r.conf.Verbose {
					...
					if matched, reason := filter.MatchesMatchers(r.conf, &res); !matched {
						if r.conf.Verbose {
							out.PrintResult(res, reason)
						}
						continue
...
func (r *SimpleRunner) recordSizeSeen(res ffuf.Result) int64 {
	host := res.Host
	if host == "" {
		host = res.Url
	}
	key := fmt.Sprintf("%s|%d", host, res.ContentLength)
	r.sizeMu.Lock()
	defer r.sizeMu.Unlock()
	r.sizeCounters[key]++
	return r.sizeCounters[key]
}
...
// Returns nil filters (no error) when calibration probes do not match the active
// matcher/filter rules, because those responses would not be shown as findings.
func (r *SimpleRunner) calibrateTarget(urlTemplate string) ([]ffuf.FilterProvider, error) {
	const calibSamples = 4
	...
	for i := 0; i < calibSamples; i++ {
		...
		if passed, _ := filter.PassesFilters(r.conf, &res); !passed {
			continue
		}
		if matched, _ := filter.MatchesMatchers(r.conf, &res); !matched {
			continue
		}
	...
	if len(samples) < 2 {
		return nil, fmt.Errorf("insufficient matching calibration samples")
	}
...
func mergedFilterValue(existing ffuf.FilterProvider, value string) string {
	if existing == nil {
		return value
	}
	current := strings.TrimSpace(existing.Repr())
	if current == "" {
		return value
	}
...
func (r *SimpleRunner) autoCalibrate(out ffuf.OutputProvider) error {
	...
	for _, f := range filters {
		...
		case *ffuf.SizeFilterEntry:
			v.Value = mergedFilterValue(r.conf.Filters["size"], v.Value)
			r.conf.Filters["size"] = v
			if !r.conf.Quiet && r.conf.OutputMode == "normal" {
				out.Info(fmt.Sprintf("Auto-filter: response size %s bytes", v.Value))
			}
		case *ffuf.WordFilterEntry:
			v.Value = mergedFilterValue(r.conf.Filters["word"], v.Value)
			r.conf.Filters["word"] = v
			if !r.conf.Quiet && r.conf.OutputMode == "normal" {
				out.Info(fmt.Sprintf("Auto-filter: response words %s", v.Value))
			}
		case *ffuf.LineFilterEntry:
			v.Value = mergedFilterValue(r.conf.Filters["line"], v.Value)
			r.conf.Filters["line"] = v
			if !r.conf.Quiet && r.conf.OutputMode == "normal" {
