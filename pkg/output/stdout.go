...
type Stdoutput struct {
	...
	// Warning deduplication — same message is only printed once.
...
func (s *Stdoutput) Banner() {
	if s.conf.OutputMode != "normal" || s.conf.Json {
		return
	}
...
// Info prints a cyan [INFO] message — only in normal mode.
...
// Error prints a red [ERR] message. Errors bypass all suppression and always
// show in normal mode. They are suppressed only in silent mode.
func (s *Stdoutput) Error(msg string) {
	if s.conf.OutputMode == "silent" || s.conf.Json {
		return
	}
	// In live mode errors would disrupt the spinner, so only show in normal mode.
	if s.conf.OutputMode != "normal" {
		return
	}
	s.mu.Lock()
...
// Warning prints a yellow [WARN] message — deduplicated so the same message
// is only ever printed once per run. Only shown in normal mode.
func (s *Stdoutput) Warning(msg string) {
	if s.conf.OutputMode != "normal" || s.conf.Json {
		return
	}
	s.warnMu.Lock()
	if _, seen := s.warnSeen[msg]; seen {
		s.warnMu.Unlock()
		return
	}
	s.warnSeen[msg] = struct{}{}
	s.warnMu.Unlock()
...
// CatchAll prints an aggregated catch-all suppression notice.
// Format: [CATCH-ALL] example.com — 7177 B (seen 5x)
func (s *Stdoutput) CatchAll(host string, size int64, seen int64) {
	if s.conf.OutputMode != "normal" || s.conf.Json || s.conf.Quiet {
		return
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.conf.Colors {
		fmt.Fprintf(os.Stderr, "\r\033[2K%s[CATCH-ALL]%s %s — %d B (seen %dx)\n",
			ANSI_YELLOW, ANSI_CLEAR, host, size, seen)
	} else {
		fmt.Fprintf(os.Stderr, "\r\033[2K[CATCH-ALL] %s — %d B (seen %dx)\n", host, size, seen)
	}
}
